...
function obdelajDregljaj(socket) {
  socket.on('dregljaj', function (vzdevek) {
    if (vzdevek) {
      var socketIdNaslovnika = null;
      for (var id in vzdevkiGledeNaSocket) {
        if (vzdevek == vzdevkiGledeNaSocket[id]) {
          socketIdNaslovnika = id;
          break;
        }
      }
      // Check if user has been found and not the sender.
      if (socketIdNaslovnika != null && socketIdNaslovnika != socket.id) {
        // Emit signal to poke.
        io.sockets.sockets[socketIdNaslovnika].emit('dregljaj', {
        	vzdevek: vzdevkiGledeNaSocket[socket.id],
        });
        // Notify sender about success.
        io.sockets.sockets[socket.id].emit("sporocilo", {
        	besedilo: "Dregljaj za " + vzdevek,
        });
      } else {
        // Notify sender about not found error.
        io.sockets.sockets[socket.id].emit('sporocilo', {
          besedilo: "Dregljaja uporabniku z vzdevkom '" + vzdevek + "' ni bilo mogoče posredovati."
        });
      }
    } else {
      // Notify sender about command error.
      io.sockets.sockets[socket.id].emit("sporocilo", {
